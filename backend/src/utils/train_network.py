...
def update_weights(optimizer: torch.optim.Optimizer, network: Network, batch: list) -> tuple[torch.Tensor, dict]:
    """
...
def train_network(config: Config, network: Network, replay_buffer: ReplayBuffer, iterations: int, logger: WandbLogger = None) -> torch.Tensor:
    """
    ...
    for e in range(config.epochs):
        ...
        # Compute loss
        loss, loss_dict = update_weights(optimizer=optimizer, network=network, batch=batch)
        ...
        if logger is not None:
            logger.log_losses(
                ...
                step=network.tot_training_steps
            )
            logger.log_learning_rate(lr, network.tot_training_steps)
